...
```bash
curl -fsSL https://<your-deplo-host>/install | bash
```

The script is generated by `lib/install-script.ts` and served from the public
`/install` route. It is idempotent: secrets are generated once and stored in
`/opt/deplo/.env`, so re-running it never rotates them. After it finishes, point
your domain at the server and finish setup in the browser.
...
app/install       public installer script endpoint
