...
export function defaultVariant(template: { slug?: string; variants: readonly ApiTemplateVariant[] }) {
  const variant = template.variants.find(({ slug }) => slug === DEFAULT_VARIANT_SLUG);
  if (!variant) throw new Error(`Template ${template.slug ?? ""} has no default variant.`);
  return variant;
}
 ...
 */
export type CatalogTemplateVariant = Omit<ApiTemplateVariant, "logo" | "images"> & {
  logo: string;
  images: string[];
};
