...
export function refusalMessage(r: PreviewRefusal): string {
  switch (r.kind) {
    ...
    case "cap":
      return `This app already has ${r.max} previews running, its limit. Destroy one to make room.`;
    case "fork-denied":
...
export async function openOrSyncPreview(
  appId: string,
  pr: PullRequestFacts,
  opts: { actor: string; /** A manual deploy approves a fork implicitly. */ approve?: boolean } = {
    actor: "github",
  },
  ...
  return withKeyedLock(`preview:${appId}:${pr.number}`, async () => {
    ...
    if (existing) {
      ...
      // The cap only ever refuses a NEW preview. An existing one keeps building:
      // it is already occupying the slot, and refusing its next push would leave
      // a stale URL live on the pull request.
      const open = await countOpenPreviews(appId);
      if (open >= settings.maxActive) {
        return {
          previewId: null,
          deploymentId: null,
          refusal: { kind: "cap", max: settings.maxActive },
        };
      }
      const server = (await getServerById(app.serverId)) ?? undefined;
...
async function countOpenPreviews(appId: string): Promise<number> {
  const rows = await getDb()
    ...
    .from(appPreviewsTable)
    .where(
      and(eq(appPreviewsTable.appId, appId), eq(appPreviewsTable.state, "open")),
    );
  return rows[0]?.n ?? 0;
}
