...
function serverIdOf(project: App): string {
 ...
 */
async function resolveCloneSecretUrl(project: App): Promise<string> {
  if (
 ...
 */
async function buildUploadTar(project: App): Promise<Buffer> {
...
export async function agentStopDev(project: App): Promise<void> {
...
export async function agentTeardownDev(project: App): Promise<void> {
  const conn = await connectAgent(serverIdOf(project));
  try {
...
export async function agentStopTunnel(project: App): Promise<void> {
  const conn = await connectAgent(serverIdOf(project));
  try {
