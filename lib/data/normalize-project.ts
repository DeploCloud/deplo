 ...
 *
 * Extracted from `lib/data/projects.ts` so BOTH the live read path AND the
 * project-graph backfill apply the IDENTICAL normalization before a legacy row is
 * exploded into the strict NOT-NULL child tables — the same anti-drift split as
 * `project-graph-rows.ts`. These touch no store/db and import only pure helpers
 * (`newId`, `normalizeBuildConfig`), so the backfill can import them without
 * pulling in the `server-only` data layer. `projects.ts` re-exports them so its
 ...
 */
export function normalizeProject<T extends Project>(p: T): T {
