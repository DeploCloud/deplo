 ...
 * this seeds those tables directly, the same way `project-graph-test-helpers` seeds
 * the project graph.
 *
...
export async function seedDevSshUser(
  db: TestDb,
  opts: Partial<DevSshUser> & { id: string; projectId: string; username: string },
): Promise<DevSshUser> {
  const u: DevSshUser = {
    id: opts.id,
...
export async function seedActivity(
  ...
  const a: Activity = {
    id: opts.id,
    teamId: opts.teamId,
    type: (opts.type ?? "project") as ActivityType,
