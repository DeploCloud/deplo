...
import { createBackup, runProjectBackup, deleteBackup } from "./backups";
import {
  folderCapabilities,
  setFolderGrant,
  listFolderGrants,
 ...
 * that capability ON THE FOLDER (owner, grant, or super-user). Top-level projects
 * stay team-only.
 *
...
beforeEach(async () => {
  ...
  await seedProject(db, { id: PRJ_IN, teamId: TEAM_A });
  await seedProject(db, { id: PRJ_TOP, teamId: TEAM_A });
  // Move PRJ_IN into the folder (seedProject seeds folderId=null).
  await db
    .update(projectsTable)
    .set({ folderId: FLD })
    .where(eq(projectsTable.id, PRJ_IN));
});
...
test("a team member without folder access can't act on a project inside the folder", async () => {
  ...
  const row = (await db.select().from(projectsTable).where(eq(projectsTable.id, PRJ_IN)))[0]!;
...
test("the same member CAN act on a TOP-LEVEL project (team caps govern)", async () => {
  await as(MEMBER, async () => {
    await renameProject(PRJ_TOP, "renamed-top");
  });
  const row = (await db.select().from(projectsTable).where(eq(projectsTable.id, PRJ_TOP)))[0]!;
...
test("the folder owner can act on a project inside their folder", async () => {
  await as(OWNER, async () => {
    await renameProject(PRJ_IN, "owner-renamed");
  });
  const row = (await db.select().from(projectsTable).where(eq(projectsTable.id, PRJ_IN)))[0]!;
...
test("a grantee with folder `deploy` can act; without it they can't", async () => {
  ...
  await as(GRANTEE, async () => {
    await renameProject(PRJ_IN, "grantee-renamed");
  });
  assert.equal(
    (await db.select().from(projectsTable).where(eq(projectsTable.id, PRJ_IN)))[0]!.name,
    "grantee-renamed",
  );
  ...
  await as(GRANTEE, async () => {
    await assert.rejects(
      () => renameProject(PRJ_IN, "grantee-again"),
      /not found|permission/i,
    );
  });
});
...
test("manage_infra: the folder owner CAN back up a project inside their folder", async () => {
  ...
  assert.equal(dto.targetKind, "project");
