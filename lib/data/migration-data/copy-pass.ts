...
/** What the two copy passes counted, read by the caller once both have run. */
export interface CopyTally {
  moved: number;
  failed: number;
  empty: number;
  /** Of `empty`, the ones that do not EXIST over there. See VolumeCopyResult.missing. */
  missing: number;
  /** Of `missing`, the ones belonging to a service that IS running over there, which
   *  is data that should have come across. Kept apart from `failed` because that one
   *  also decides whether a database is started again. */
  notCopied: number;
  /** See DataMoveResult.sourceGone: the difference between "this volume did not come
   *  across" and "stop the whole migration". */
  sourceGone: boolean;
  /** What did NOT arrive, kept apart from the notes (which also carry advice) so it
...
/** Everything both passes need about the service being cut over. */
export interface CopyContext {
  ...
  svc: SourceService;
  landed: Landed;
  /** Whether the source is still up on the panel. */
  running: boolean;
...
async function recordEmpty(
  ...
  // somewhere else. Left as a `skipped` line, the app came up on empty storage
  // with nothing anywhere refusing to let it.
  ...
  await record(
    ctx,
    sourceName,
    // Data that did not come across is a FAILURE, never a line the summary folds
    // into "skipped": a run that ends `failed: 0` over a volume nobody copied
    // reads as a clean migration.
...
async function recordFailure(
  ...
  // Cancelled, not broken: leave every remaining volume alone and let the stop
  // that asked for it do the undoing.
...
/** Copy every paired volume into the imported resource, one report line each. */
...
/** Copy every paired host directory, refusing the ones a copy would wipe for someone else. */
export async function copyBindMounts(
  ...
  for (const bind of binds) {
    ...
    // platform wrote. Copying it over itself would be a wipe followed by a restore
    // of what was just wiped - all risk, no movement.
    if (
    ...
    // writes, so going ahead would erase a stranger's data and report a clean run.
    ...
    if (
      ...
      // The same directory on the same source machine, mounted by two services of
      // this run: whichever comes first fills it, the other keeps it.
