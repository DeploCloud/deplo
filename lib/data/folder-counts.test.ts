...
import { createFolder, listFolders, moveServiceToFolder } from "./folders";
import { createProject, listProjects, moveServiceToProject } from "./projects";

/**
 * Integration tests for the LIVE COUNTS on folder and project tiles against
 * pglite. Proves the subtree semantics: a folder's serviceCount covers
 * everything nested anywhere beneath it (not just direct children), and a
 * project's serviceCount also reaches services filed inside a legacy
...
test("folder serviceCount covers the whole subtree, subfolderCount stays immediate", async () => {
  await asOwner(async () => {
    ...
    await seedService(db, { id: "svc_top", teamId: TEAM_A });
    await seedService(db, { id: "svc_mid", teamId: TEAM_A });
    await seedService(db, { id: "svc_leaf", teamId: TEAM_A });
    await moveServiceToFolder("svc_top", top.id);
    await moveServiceToFolder("svc_mid", mid.id);
    await moveServiceToFolder("svc_leaf", leaf.id);

    const byId = new Map((await listFolders()).map((f) => [f.id, f]));
    assert.equal(byId.get(top.id)?.serviceCount, 3, "top sees its whole subtree");
    assert.equal(byId.get(mid.id)?.serviceCount, 2, "mid sees itself + leaf");
    assert.equal(byId.get(leaf.id)?.serviceCount, 1);
...
test("an empty parent over populated subfolders no longer reads 0", async () => {
  await asOwner(async () => {
    ...
    await seedService(db, { id: "svc_a1", teamId: TEAM_A });
    await seedService(db, { id: "svc_a2", teamId: TEAM_A });
    await seedService(db, { id: "svc_b1", teamId: TEAM_A });
    await moveServiceToFolder("svc_a1", a.id);
    await moveServiceToFolder("svc_a2", a.id);
    await moveServiceToFolder("svc_b1", b.id);

    const got = (await listFolders()).find((f) => f.id === parent.id)!;
    assert.equal(got.serviceCount, 3, "nothing direct, everything via subfolders");
    assert.equal(got.subfolderCount, 3);
  });
});

test("project serviceCount reaches services inside a legacy folder-in-project subtree", async () => {
  await asOwner(async () => {
    ...
    await seedService(db, { id: "svc_direct", teamId: TEAM_A });
    await seedService(db, { id: "svc_legacy", teamId: TEAM_A });
    await seedService(db, { id: "svc_nested", teamId: TEAM_A });
    await seedService(db, { id: "svc_outside", teamId: TEAM_A });
    await moveServiceToProject("svc_direct", p.id);
    await moveServiceToFolder("svc_legacy", legacy.id);
    await moveServiceToFolder("svc_nested", nested.id);
