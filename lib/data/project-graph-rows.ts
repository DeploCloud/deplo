...
export type ProjectBuildRow = typeof projectBuild.$inferSelect;
...
export type SharedEnvGroupProjectRow =
  typeof sharedEnvGroupProjects.$inferSelect;
export type SharedEnvGroupTargetRow = typeof sharedEnvGroupTargets.$inferSelect;

type ProjectInsert = typeof projects.$inferInsert;
type ProjectBuildInsert = typeof projectBuild.$inferInsert;
 ...
 * {@link assembleProject}. `build`/`dev` are 1-to-1 (absent ⇒ null, the
...
/** Reassemble a {@link BuildConfig} from the `project_build` (+ method-settings) rows. */
export function assembleBuild(
  build: ProjectBuildRow,
  ms: ProjectBuildMethodSettingsRow | null,
): BuildConfig {
  return {
...
/** Reassemble a {@link DevConfig} from the `project_dev` row (null ⇒ absent). */
export function assembleDev(dev: ProjectDevRow): DevConfig {
  return {
    enabled: dev.enabled,
    status: dev.status,
...
function assembleRepo(row: ProjectRow): Project["repo"] {
  if (row.repoProvider == null) return null;
  return {
    provider: row.repoProvider as NonNullable<Project["repo"]>["provider"],
    url: row.repoUrl ?? "",
    repo: row.repoRepo ?? "",
...
function assembleUpload(row: ProjectRow): Project["upload"] {
...
function volumeRowToMount(v: ProjectVolumeRow): VolumeMount {
  if (v.type === "host") {
    return {
      id: v.volumeId,
      type: "host",
...
/** The flat `projects` row for a {@link Project} (children handled separately). */
export function projectToRow(p: Project): ProjectInsert {
  return {
    id: p.id,
    name: p.name,
    slug: p.slug,
...
export function buildToRow(projectId: string, b: BuildConfig): ProjectBuildInsert {
  return {
    projectId,
...
/**
 * The 1-to-1 `project_build_method_settings` row. Each {@link BuildMethodSettings}
...
export function methodSettingsToRow(
...
): ProjectBuildMethodSettingsInsert {
  const cols = {
  ...
  return { projectId, ...cols };
}

export function devToRow(projectId: string, dev: DevConfig): ProjectDevInsert {
  return {
    projectId,
    enabled: dev.enabled,
    status: dev.status,
...
export function volumesToRows(
  ...
  return (volumes ?? []).map((v, position) => ({
    ...
    type: v.type === "host" || v.type === "project" ? v.type : null,
    name: v.name,
    projectPath: v.type === "project" ? v.projectPath : null,
...
/** The full FK-ordered row-set for a normalized {@link Project}. */
export function projectToRowSet(p: Project): ProjectRowSet {
  const ms = methodSettingsToRow(p.id, p.build.methodSettings);
  return {
    project: projectToRow(p),
...
export function assembleSharedEnvGroup(
  ...
  projectIds: SharedEnvGroupProjectRow[],
  ...
  return {
    ...
    projectIds: projectIds.map((p) => p.projectId),
    targets: targets.map((t) => t.target as EnvTarget),
    createdAt: row.createdAt,
    updatedAt: row.updatedAt,
  };
