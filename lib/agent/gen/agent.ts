...
export interface DeployUpload {
  ...
  request?: DeployRequest | undefined;
  contextChunk?: Uint8Array | undefined;
}
...
function createBaseDeployUpload(): DeployUpload {
  return { request: undefined, contextChunk: undefined };
}

export const DeployUpload: MessageFns<DeployUpload> = {
  encode(message: DeployUpload, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.request !== undefined) {
      DeployRequest.encode(message.request, writer.uint32(10).fork()).join();
    }
    if (message.contextChunk !== undefined) {
      writer.uint32(18).bytes(message.contextChunk);
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): DeployUpload {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const previousRecursionDepth = (reader as any).__tsProtoDecodeDepth ?? 0;
    if (previousRecursionDepth >= 100) {
      throw new globalThis.Error("protobuf decode recursion limit exceeded");
    ...
    try {
      const end = length === undefined ? reader.len : reader.pos + length;
      const message = createBaseDeployUpload();
      while (reader.pos < end) {
        const tag = reader.uint32();
        switch (tag >>> 3) {
          case 1: {
            ...
            message.request = DeployRequest.decode(reader, reader.uint32());
            continue;
          }
          case 2: {
            if (tag !== 18) {
            ...
            message.contextChunk = reader.bytes();
            continue;
          }
        }
        if ((tag & 7) === 4 || tag === 0) {
  ...
  fromJSON(object: any): DeployUpload {
    return {
      request: isSet(object.request) ? DeployRequest.fromJSON(object.request) : undefined,
      contextChunk: isSet(object.contextChunk)
        ? bytesFromBase64(object.contextChunk)
        : isSet(object.context_chunk)
        ? bytesFromBase64(object.context_chunk)
        : undefined,
    };
  },

  toJSON(message: DeployUpload): unknown {
    const obj: any = {};
    if (message.request !== undefined) {
      obj.request = DeployRequest.toJSON(message.request);
    }
    if (message.contextChunk !== undefined) {
      obj.contextChunk = base64FromBytes(message.contextChunk);
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<DeployUpload>, I>>(base?: I): DeployUpload {
    return DeployUpload.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<DeployUpload>, I>>(object: I): DeployUpload {
    const message = createBaseDeployUpload();
    message.request = (object.request !== undefined && object.request !== null)
      ? DeployRequest.fromPartial(object.request)
      : undefined;
    message.contextChunk = object.contextChunk ?? undefined;
    return message;
  },
};
      ...
      binaries: isObject(object.binaries)
        ? (globalThis.Object.entries(object.binaries) as [string, any][]).reduce(
          (acc: { [key: string]: ArchBinary }, [key, value]: [string, any]) => {
            globalThis.Object.defineProperty(acc, key, {
              value: ArchBinary.fromJSON(value),
              enumerable: true,
              configurable: true,
              writable: true,
            });
...
export const AgentService = {
   ...
   */
  deployStream: {
    path: "/deplo.agent.v1.Agent/DeployStream" as const,
    requestStream: true as const,
    responseStream: true as const,
    requestSerialize: (value: DeployUpload): Buffer => Buffer.from(DeployUpload.encode(value).finish()),
    requestDeserialize: (value: Buffer): DeployUpload => DeployUpload.decode(value),
    responseSerialize: (value: DeployEvent): Buffer => Buffer.from(DeployEvent.encode(value).finish()),
    responseDeserialize: (value: Buffer): DeployEvent => DeployEvent.decode(value),
  },
  /**
...
export interface AgentServer extends UntypedServiceImplementation {
   ...
   * "deploy.context_stream"; events and reattach behave exactly as Deploy's.
   */
  deployStream: handleBidiStreamingCall<DeployUpload, DeployEvent>;
  /**
   * Lifecycle control over an already-deployed stack. These replace the direct
   * stop/start/destroy docker calls in lib/deploy/build.ts for agent-owned
   * stacks. (Wired in a later step; defined now so the boundary is complete.)
...
export interface AgentClient extends Client {
   ...
   * "deploy.context_stream"; events and reattach behave exactly as Deploy's.
   */
  deployStream(): ClientDuplexStream<DeployUpload, DeployEvent>;
  deployStream(options: Partial<CallOptions>): ClientDuplexStream<DeployUpload, DeployEvent>;
  deployStream(metadata: Metadata, options?: Partial<CallOptions>): ClientDuplexStream<DeployUpload, DeployEvent>;
  /**
   * Lifecycle control over an already-deployed stack. These replace the direct
   * stop/start/destroy docker calls in lib/deploy/build.ts for agent-owned
   * stacks. (Wired in a later step; defined now so the boundary is complete.)
