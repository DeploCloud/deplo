/**
 * Copy-paste Linux installer generator (Coolify-style: zero terminal config).
 * Served as text/x-shellscript from /install so users can run:
 *   curl -fsSL https://<host>/install | bash
 *
 * The script is idempotent: it installs Docker, provisions the shared `deplo`
 * network, brings up Traefik with automatic Let's Encrypt TLS, a private
 * Postgres, then the Deplo control plane. Secrets are generated once and stored
 * in /opt/deplo/.env so re-running the script never rotates them.
 *
 * Compose files use quoted heredocs (literal content) and rely on
 * `docker compose --env-file` for variable substitution, which keeps the
 * generated shell free of fragile escaping.
 */
...
export function generateInstallScript(opts: {
  baseUrl: string;
  version?: string;
  acmeEmail?: string;
}): string {
  const version = opts.version ?? "latest";
  const acmeEmail = opts.acmeEmail ?? "admin@example.com";
  return `#!/usr/bin/env bash
#
# Deplo installer
# Usage:  curl -fsSL ${opts.baseUrl}/install | bash
#
