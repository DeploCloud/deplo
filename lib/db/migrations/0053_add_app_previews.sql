...
--> statement-breakpoint
-- NULL ⇒ PREVIEW_MAX_ACTIVE_DEFAULT. A cap, not a quota: over it a NEW preview is
-- refused with a reason the user can read — a running preview is never silently
-- evicted, because that URL is live in a pull request somebody is testing.
