...
const DOKPLOY_VIEWBOX = "86 51.88 428.15 363.12";
const DOKPLOY_PATHS = [
...
/**
 * Both marks are inlined rather than loaded from `public/`, for the reason
 * `components/logo.tsx` gives about Deplo's own: an `<image href>` renders the
 * file's baked-in fill, and Dokploy's is black.
 */
function dokployPaths() {
  return DOKPLOY_PATHS.map((d, i) => (
    <path key={i} d={d} fill="currentColor" stroke="currentColor" />
  ));
}

/** The Dokploy mark on its own, for a line of text. Theme-aware. */
export function DokployMark({ className }: { className?: string }) {
  return (
    <svg
...
const LABEL: Record<MigrationState, string> = {
  ...
  install: "A cable being run from the Dokploy server toward Deplo",
  review: "The cable from Dokploy to Deplo, nearly complete",
  moving: "Data travelling along the cable from Dokploy to Deplo",
  done: "The Deplo server lit up, the Dokploy server switched off",
};
